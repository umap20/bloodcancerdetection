...
def app(df, X, y):
    ...
    # Set the page title
    st.title("Visualise Blood Cancer Demographics")
    ...
    if st.checkbox("Platelets vs RBC"):
        sns.color_palette("rocket", as_cmap=True)
        ax=sns.scatterplot(x="plt",y="rbc",data=df)
        st.pyplot()

    if st.checkbox("Show Sample Results"):
        safe = (df['phenotype'] == 0).sum()
        prone = (df['phenotype'] == 1).sum()
        data = [safe,prone]
